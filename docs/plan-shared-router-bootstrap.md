...
- **Unit 5 — Python binding (`bindings/python`)**: ✅ done. cffi (ABI mode,
  `ffi.dlopen`) over the same `router-ffi` cdylib the Go binding links —
  the ABI now exercised two genuinely different ways (linked cgo vs dlopen'd
  cffi) before it freezes. `ffi.new_handle`/`ffi.from_handle` park the
  per-dispatch session in `host_ctx` (the cffi analog of Go's `cgo.Handle`);
  one `ffi.callback` trampoline routes by `(host_ctx, callback_id)` and
  catches every Python exception so none crosses into Rust. Runs the **same**
  `conformance/features` + `.txtpb` fixtures via **pytest-bdd** (17 scenarios
  green) plus the binding-only 169-case property sweep and a GIL-threaded
  concurrent-dispatch isolation test. Python protobuf generated with the
  `protocolbuffers/python` buf plugin (pinned) under **stock protobuf 7.x**;
  google.rpc/google.api from **googleapis-common-protos**, sererr generated
...
Two unit-5 findings (surfaced building the Python binding; recorded so they
don't get rediscovered each binding):

- **Generated-import `io` collision (Python) — solved by an import
  rewrite.** The `protocolbuffers/python` plugin emits cross-file imports by
  proto PATH: a file under `io/angzarr/v1/` imports a sibling as
  `from io.angzarr.v1 import types_pb2`. That top-level `io` package is
  **unimportable** — stdlib `io` is already loaded (and is not a package)
  before any user code runs, so `from io.angzarr...` raises "io is not a
  package". (client-python never hit this: its package predates the
  `io.angzarr` rename — it was `angzarr_client.proto.angzarr.v1`. Go is
  immune — Go import paths don't collide with anything.) `buf.gen.yaml`
  generates into `angzarr_router_ffi/gen/`, then
  `scripts/fixup_gen_imports.py` reroots the binding's own packages
  (`io.`/`sererr.`/`test.`) under the `angzarr_router_ffi.gen.` prefix;
  `google.*` is left alone (it resolves from the protobuf runtime +
  googleapis-common-protos). The rewrite is part of `py-binding-gen`.
  protobuf stays **stock 7.x** (plugin pinned to the matching tag), as §4
  resolved — the collision is orthogonal to the runtime-version question.
- **No generated code is committed, in any language — regenerate on need.**
  Generated protobuf code (`bindings/go/gen`, `bindings/python/.../gen`) is
  `.gitignore`d and regenerated by the `*-binding-gen` recipes, which the
  `build`/`test`/`lint` recipes now depend on. This was applied
  retroactively to the Go binding too (its `gen/` was previously committed).
  Mirrors client-python's own convention (generated `_pb2.py` are gitignored
  there as well).
...
| 5 | Python binding + pytest-bdd harness | angzarr-router (`bindings/python`) | ✅ **done** — 17 scenarios green via pytest-bdd against the FFI core (cffi dlopen); 169-case property sweep green; GIL-threaded concurrent-dispatch isolation exercised; no old client linked |
